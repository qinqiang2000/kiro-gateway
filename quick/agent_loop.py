...
async def _run_one_round(converse_input: Dict, message_id: str, model: str) -> ConverseAggregator:
...
async def run_websearch_loop(
    converse_input: Dict, message_id: str, model: str
    ...
    non-web_search tool_use, which is returned as-is for the client to handle).
    ...
    for round_idx in range(MAX_SEARCH_ROUNDS):
        aggregator = await _run_one_round(converse_input, message_id, model)
        if aggregator.error:
